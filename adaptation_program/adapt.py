...
- 이 규칙은 "③ 성분·수치 팩트" 섹션뿐 아니라 "⑤ 실제 사용·섭취 후기(찐후기)" 섹션에도
  동일하게 적용됩니다. 후기 속에서 언급하는 제품의 구체적 특징(포장, 정제 크기·질감 등)도
  반드시 제품 필수 가이드 근거가 있어야 합니다.
...
3. 본문 — 목차와 정확히 같은 순서로, 각 섹션은 큰따옴표 인용구 소제목 "이렇게" 표기 후
   ...
   - ⑤ 마지막 "실제 사용·섭취 후기" 섹션은 특히 풍성하게 씁니다. 형식적으로 짧게 끝내지
     ...
     이 "찐후기"도 "제품 사실 정보 출처 규칙"을 그대로 따릅니다 — 포장 형태, 정제 크기·질감,
     섭취 타이밍처럼 후기 속에서 언급하는 제품의 구체적 특징은 반드시 제품 필수 가이드에
     실제로 나온 내용이어야 하며, 가이드에 없는 제품 특징(맛, 냄새, 색, 크기 등)은
     지어내지 않습니다. 페르소나 경험(운동·업무·식습관 등)은 자유롭게 창작하되, 그 경험이
     "제품과 만나는 지점"의 사실관계는 가이드에 근거해야 합니다.
