...
- 제품 필수 가이드는 오직 "⑤ 실제 사용·섭취 후기(찐후기, 제품단)" 섹션에서만 사용합니다.
  도입부와 ①②③④(정보단)에는 제품 필수 가이드의 내용이든 제품명이든 어떤 형태로도
  등장하지 않습니다 — 아래 "구조 및 포맷"의 정보단/제품단 규칙을 참고하세요. ⑤에서
  언급하는 제품의 구체적 특징(포장, 정제 크기·질감 등)은 반드시 제품 필수 가이드 근거가
  있어야 합니다.
...
3. 본문 — 목차와 정확히 같은 순서로, 각 섹션은 큰따옴표 인용구 소제목 "이렇게" 표기 후
   ...
   - **제품단(⑤ "실제 사용·섭취 후기")에서만 비로소 제품이 등장합니다.** 형식적으로 짧게
     ...
     반드시 제품 필수 가이드에 실제로 나온 내용이어야 하며, 가이드에 없는 제품 특징(맛,
     냄새, 색, 크기 등)은 지어내지 않습니다. 페르소나 경험(운동·업무·식습관 등)은 자유롭게
     창작하되, 그 경험이 "제품과 만나는 지점"의 사실관계는 가이드에 근거해야 합니다.
4. `---` 구분선
