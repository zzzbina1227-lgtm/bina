...
INFO_SECTION_RATIO = 0.7
REVIEW_SECTION_RATIO = 0.3
REVIEW_SECTION_CHAR_TARGET = int(BODY_CHAR_LIMIT * REVIEW_SECTION_RATIO)
TOC_MARK = "목차"
...
제품에 대한 모든 구체적 사실(특허, 원료명/브랜드명, 성분 리스트와 함량, 기능성 문구,
포장 형태 등)의 근거는 다음 둘 중 하나이며, 이 둘 외에는 어디서도 가져오지 않습니다.
- "제품 필수 가이드"가 별도로 제공된 경우: 그 가이드에 실제로 적힌 내용만 사실 근거로
  삼습니다. 가이드에 있는 표현(예: "17종 비타민 미네랄 100%*", "국내 2종 특허",
  "LIPOPRIME", "23중 기능성")은 각주(*1일 영양성분기준치 대비 등)까지 포함해 정확하게
  반영합니다.
- 별도 가이드가 없는 경우: "참고 원고"가 이미 필수 가이드 내용이 반영된 완성 원고일 수
  있습니다. 이때 원고에 이미 적힌 제품 관련 사실(수치, 성분, 특허, 포장 등)은 이미 검증된
  근거로 보고 **절대 임의로 바꾸거나 수정하거나 새로 지어내지 않습니다.** 표현과 문장
  구조, 구성만 새롭게 재창작하고, 원고에 있는 사실 그 자체(숫자, 성분명, 고유명사 등)는
  그대로 보존합니다.
- 위 두 경우 모두, 주어진 자료(가이드 또는 원고)에 없는 수치·성분·기능성·인증·특허 내용을
  임의로 만들어내거나 추측해서 채우지 않습니다.
- 두 자료 모두 없다면(원고에도 제품 관련 구체적 사실이 없다면), 확정적인 수치·성분명을
  서술하지 않고 조건부 표현("제품 표시사항을 확인해보시길 권해요" 등)으로 대신합니다.
- 사실 근거를 본문 표현으로 옮길 때는 "법적 준수" 규칙(의약품 오인 표현 금지, 식약처 인정
  범위 내 조건부 서술 등)을 벗어나지 않도록 문구를 다듬습니다.
- 이 사실 근거(가이드 또는 원고에 이미 반영된 내용)는 오직 "⑤ 실제 사용·섭취 후기
  (찐후기, 제품단)" 섹션에서만 사용합니다. 도입부와 ①②③④(정보단)에는 제품 관련 사실이든
  제품명이든 어떤 형태로도 등장하지 않습니다 — 아래 "구조 및 포맷"의 정보단/제품단 규칙을
  참고하세요.
...
- 가이드나 정보를 인용할 때 "~라는 설명이 있더라고요", "~라고 안내되어 있어요",
  "~라고 표시돼 있었어요", "~라고 알려져 있어요"처럼 출처를 매번 기계적으로 붙이는
  정형화된 문구를 반복하지 않습니다. 이런 패턴이 반복되면 AI가 쓴 것처럼 티가 납니다.
  사실을 자기 목소리로 자연스럽게 녹여서 말하듯 서술하고(예: "확인해보니 ~더라고요"
  대신 그냥 "~하더라고요", "~였어요"로 바로 서술), 꼭 출처를 밝혀야 할 때만 표현을
  매번 다르게 바꿔 씁니다.
...
1. 도입부 — 계절감/일상 장면에서 자연스럽게 시작해 고민·문제의식으로 이어지는 3~4개 문단.
   소제목 없이 바로 시작합니다. 제품·브랜드명은 언급하지 않습니다. (제목은 본문에 쓰지
   않습니다 — 8번 제목 후보 중 하나를 골라 네이버 에디터의 별도 제목란에 입력하는 방식이므로
   본문 텍스트에는 포함하지 않습니다.)
...
3. 본문 — 목차와 정확히 같은 순서로, 각 섹션은 큰따옴표 인용구 소제목 "이렇게" 표기 후
   빈 줄, 짧은 줄바꿈 문단들로 구성합니다. 도입부와 ①~④를 합쳐 "정보단", ⑤를 "제품단"이라
   부르며, 아래 규칙을 지킵니다.
   - **정보단(도입부, ①~④)과 제품단(⑤)의 분량 비율은 항상 약 {int(INFO_SECTION_RATIO*100)}:
     {int(REVIEW_SECTION_RATIO*100)}을 지킵니다.** {BODY_CHAR_LIMIT}자 기준으로 정보단
     전체는 약 {int(BODY_CHAR_LIMIT*INFO_SECTION_RATIO)}자, 제품단(⑤)은 약
     {REVIEW_SECTION_CHAR_TARGET}자 내외가 되도록 배분합니다.
   - **정보단(도입부, ①②③④)에는 제품 이야기가 절대 나오면 안 됩니다.** 제품명, 브랜드명,
     "이 제품은", "저는 이 제품을" 같은 표현, 제품 필수 가이드에서 가져온 특허·성분표·함량·
     기능성 수치 등 제품과 연결되는 어떤 내용도 이 네 섹션에는 쓰지 않습니다. 순수하게
     주제에 대한 일반적인 정보(누구나 아는 상식, 일반적인 영양학 지식, 공신력 있는
     일반 기준 등)만 다룹니다.
     - ①③④ 섹션은 담백하고 간결하게 씁니다.
     - ② "정의·개념 설명" 섹션은 한두 줄로 뭉뚱그리지 않습니다. 이 주제에 일반적으로
       필요한 성분(비타민 종류별, 미네랄 종류별 등)이 무엇이고 각각 몸에서 어떤 역할을
       하는지(예: 에너지 대사, 면역, 뼈 건강, 항산화 등) 최소 3~4가지 이상 구체적으로
       짚어주는 설명을 포함해 충실하게 작성합니다(제품 가이드의 특정 제품 수치가 아닌
       일반적으로 알려진 정보 기준).
   - **제품단(⑤ "실제 사용·섭취 후기")에서만 비로소 제품이 등장합니다.** 형식적으로 짧게
     끝내지 말고, 위 "블로거 배경 설정"의 페르소나 디테일(운동 루틴, 무역회사 일상, 식습관 등
     그 글 주제와 맞는 부분)을 실제로 겪은 일처럼 구체적으로 녹여 진짜 섭취·사용 후기
     느낌으로 작성합니다. 이 "찐후기"는 "제품 사실 정보 출처 규칙"을 그대로 따릅니다 —
     포장 형태, 정제 크기·질감, 섭취 타이밍처럼 후기 속에서 언급하는 제품의 구체적 특징은
     반드시 제품 필수 가이드(또는 이미 반영된 원고) 근거가 있는 내용이어야 하며, 그 근거에
     없는 제품 특징(맛, 냄새, 색, 크기 등)은 지어내지 않습니다. 원고에 이미 있는 제품
     사실은 절대 임의로 바꾸지 말고 그대로 유지하면서, 표현과 문장 구조만 새롭게 씁니다.
     페르소나 경험(운동·업무·식습관 등)은 자유롭게 창작하되, 그 경험이 "제품과 만나는
     지점"의 사실관계는 근거 자료를 벗어나지 않아야 합니다.
4. `---` 구분선
...
def build_user_prompt(
    ...
    if guide:
        text_note = "문체·구성만 참고, 제품 사실은 아래 별도 가이드가 유일한 근거"
    else:
        text_note = "이미 제품 필수 가이드가 반영된 원고일 수 있음 — 원고에 있는 제품 관련 사실(수치·성분·특허·포장 등)은 그대로 보존하고 임의로 바꾸지 말 것, 표현/구성만 재창작"
    parts = [f"--- 참고 원고 ({text_note}) ---\n{text}\n--- 참고 원고 끝 ---"]
    if guide:
        parts.append(
...
def main() -> None:
    ...
    parser.add_argument(
        "input", nargs="?",
        help="참고할 원고 텍스트 파일 경로 (생략 시 표준입력). -g 없이 쓰면 이 원고에 이미 반영된 "
             "제품 사실을 그대로 근거로 사용하고 임의로 바꾸지 않습니다.",
    )
    parser.add_argument(
        "-g", "--guide",
        help="제품 필수 가이드 파일 경로 (제공 시 제품 사실 정보의 근거가 됨). 원고에 이미 "
             "제품 사실이 반영되어 있다면 생략 가능합니다.",
    )
